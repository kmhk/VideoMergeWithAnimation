...
class VideoManager: NSObject {
    ...
    //
    func mergeWithAnimation(arrayVideos:[AVAsset], animationType: Int, completion:@escaping Completion) -> Void {
        animatType = animationType
        ...
        for videoAsset in arrayVideos {
            ...
            do {
                ...
                if animation {
                    ...
                    if animatType == 0 { // for fade animation effect
                        layerInstruction.setOpacityRamp(fromStartOpacity: 1.0, toEndOpacity: 0.0, timeRange: CMTimeRange.init(start: endTime, duration: durationAnimation))
                    } else if animatType == 1 { // for rotate animation effect
                        let from = CGAffineTransform(rotationAngle: 0)
                        let to = CGAffineTransform(rotationAngle: 3.14)
                        layerInstruction.setTransformRamp(fromStart: from, toEnd: to, timeRange: CMTimeRange.init(start: endTime, duration: durationAnimation))
                    } else { // for scaling animation effect
                        let from = CGAffineTransform(scaleX: 1, y: 1)
                        let to = CGAffineTransform(scaleX: 0, y: 0)
                        layerInstruction.setTransformRamp(fromStart: from, toEnd: to, timeRange: CMTimeRange.init(start: endTime, duration: durationAnimation))
                    }
                }
                else {
